...
def one_hot_encoding(dataset):
    ...
    for i in range(len(dataset)):
        already_encoded.append(dataset[i][0:19])

    need_encoding = []

    for i in range(len(dataset)):
        temp = []
        for value in dataset[i][19:]:
            temp.append(value)
...
def padding(one_hot_encoded):
    ...
    padding_len = 72 - len(one_hot_encoded[0])
...
def generating(encoded, label, step):
    ...
    for i in range(len(encoded)):
        ...
        if not os.path.exists("../results/MAWILAB_Grey/" + image_dir):
            os.mkdir("../results/MAWILAB_Grey/" + image_dir)
            print(image_dir + " directory is created")

        if not os.path.exists("../results/MAWILAB_Grey/" + image_dir + "/malicious"):
            os.mkdir("../results/MAWILAB_Grey/" + image_dir + "/malicious")
            print(image_dir + "/malicious " + " directory is created")

        if not os.path.exists("../results/MAWILAB_Grey/" + image_dir + "/normal"):
            os.mkdir("../results/MAWILAB_Grey/" + image_dir + "/normal")
            print(image_dir + "/normal " + " directory is created")

        if label[i] == "anomaly":
            cv2.imwrite("../results/MAWILAB_Grey/" + image_dir + "/malicious" + "/record" + str(i) + ".jpg", data)
        elif label[i] == "normal":
            cv2.imwrite("../results/MAWILAB_Grey/" + image_dir + "/normal" + "/record" + str(i) + ".jpg", data)
