...
def loadDataset():
    ...
    for i in range(10):
        with open("../dataset/IDS/" + file_name + str(i) + ".csv", "r") as csvfile:
    ...
    for i in range(len(training_set)):
        print("Number of each dataset record: " + str(len(training_set[i])))
...
def train_random_forest(training_data):
    ...
    x_train_input = train_data[:train_index, 0:-1]
    y_train = train_data[:train_index, -1]

    x_valid_input = train_data[train_index:valid_index, 0:-1]
    y_valid = train_data[train_index:valid_index, -1]

    x_test_input = train_data[valid_index:, 0:-1]
    y_test = train_data[valid_index:, -1]
    ...
    for i in range(len(y_valid)):
        if y_valid[i] == "BENIGN":
            y_valid_label.append(0)
        else:
            y_valid_label.append(1)
    ...
    print("Number of training: " + str(len(x_train_input)))
    print("Number of validation: " + str(len(x_valid_input)))
    print("Number of testing: " + str(len(x_test_input)))

    start_time = datetime.datetime.now()

    rf.fit(x_train_input, y_train_label) # train Random Forest
    ...
    for step in range(len(training_data)):
        ...
        test_data = np.array(data[step])

        y_actual_test_label = []

        x_actual_test_input = test_data[:, 0:-1]
        y_testing = test_data[:, -1]

        for i in range(len(y_testing)):
            if y_testing[i] == "BENIGN":
                y_actual_test_label.append(0)
            else:
                y_actual_test_label.append(1)

        print("Number of actual testing: " + str(len(x_actual_test_input)))
        ...
        for i in range(len(pred_train)):
            if pred_train[i] == y_train_label[i]:
                train_true += 1
            # else:
            #     print(pred_train[i])
        ...
        for i in range(len(pred_validation)):
            if pred_validation[i] == y_valid_label[i]:
                valid_true += 1
        ...
        pred_testing_set = rf.predict(x_actual_test_input)
        testing_true = 0

        print(len(pred_testing_set))
        print(len(y_actual_test_label))
        for i in range(len(pred_testing_set)):
            if pred_testing_set[i] == y_actual_test_label[i]:
                testing_true += 1
        ...
        print(type(y_actual_test_label[0]))
        print(type(pred_testing_set[0]))
        print("Current step: " + str(step))

        draw_confusion_matrix(y_actual_test_label, pred_testing_set, testing_acc, step)
        draw_roc_curve(y_actual_test_label, pred_testing_set, step)
...
if __name__ == "__main__":
    ...
    train_random_forest(training_data)
