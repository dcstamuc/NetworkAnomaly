...
def load_dataset(dataset_name):
    ...
    with open("../dataset/IDS/" + dataset_name + ".csv", "rU") as csvReader:
...
def generateImages(encoded_train, train_label):
    ...
    for i in range(len(encoded_train)):
        ...
        image_dir = "IDS_Our_" + dataset_name
        ...
        if train_label[i] != "BENIGN":
            cv2.imwrite("../generatedImages/" + image_dir + "/malicious" + "/record" + str(i) + ".png", data)
        elif train_label[i] == "BENIGN":
            cv2.imwrite("../generatedImages/" + image_dir + "/normal" + "/record" + str(i) + ".png", data)
...
if __name__ == "__main__":
    ...
    train, train_label = load_dataset(dataset_name)
    padded_train = padding(train)
    encoded_train = encoding(padded_train)
    generateImages(encoded_train, train_label)
