...
def generating(encoded, label):
    ...
    for i in range(len(encoded)):
        ...
        if not os.path.exists("../generatedImages/UNSW_Our"):
            os.mkdir("../generatedImages/UNSW_Our")

        if not os.path.exists("../generatedImages/UNSW_Our/" + image_dir):
            os.mkdir("../generatedImages/UNSW_Our/" + image_dir)
            print(image_dir + " directory is created")

        if not os.path.exists("../generatedImages/UNSW_Our/" + image_dir + "/malicious"):
            os.mkdir("../generatedImages/UNSW_Our/" + image_dir + "/malicious")
            print(image_dir + "/malicious " + " directory is created")

        if not os.path.exists("../generatedImages/UNSW_Our/" + image_dir + "/normal"):
            os.mkdir("../generatedImages/UNSW_Our/" + image_dir + "/normal")
            print(image_dir + "/normal " + " directory is created")

        if label[i] == '1':
            cv2.imwrite("../generatedImages/UNSW_Our/" + image_dir + "/malicious" + "/record" + str(i) + ".jpg", data)
        elif label[i] == '0':
            cv2.imwrite("../generatedImages/UNSW_Our/" + image_dir + "/normal" + "/record" + str(i) + ".jpg", data)
