...
def load_dataset(step):
    ...
    with open("../dataset/IDS/" + file_name + str(step) + ".csv", "rU") as csvReader:
...
def generateImages(encoded_train, train_label, step):
    ...
    for i in range(len(encoded_train)):
        ...
        image_dir = "IDS_Grey_Train" + "_" + str(step)
        ...
        if train_label[i] != "BENIGN":
            cv2.imwrite("../results/" + image_dir + "/malicious" + "/record" + str(i) + ".jpg", data)
        elif train_label[i] == "BENIGN":
            cv2.imwrite("../results/" + image_dir + "/normal" + "/record" + str(i) + ".jpg", data)
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        generateImages(encoded, label, i)
