...
dirName = "IDS_results"
file_name = "IDS_Our_Train"
sub_name = "ours"

def loadResultSet(step):

    malicious_total_cnt = 0
    malicious_total_list = []
    malicious_list = []
    with open("../dataset/" + dirName + "/" + sub_name + "/" + file_name + str(step) + "_partial_malicious_result.csv", "r") as csvfile:
    ...
    normal_total_list = []
    normal_list = []
    with open("../dataset/" + dirName + "/" + sub_name + "/" + file_name + str(step) + "_partial_normal_result.csv", "r") as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
    ...
    return malicious_total_cnt, malicious_total_list, malicious_list, normal_total_cnt, normal_total_list, normal_list, step
...
def getStatistics(ms, y_label, y_predicted, acc, step):
    ...
    with open("../results/" + file_name + str(step) + "_statistical_result.csv", "w") as csvfile:
...
def draw_confusion_matrix(y_label, y_predicted, acc, step):
    ...
    plt.savefig("../results/" + file_name + str(step) + "_cm.png")
    plt.show()
...
def draw_roc_curve(label, predicted, step):
    ...
    plt.savefig("../results/" + file_name + str(step) + "_roc.png")
    plt.show()


if __name__ == "__main__":
    malicious_total_cnt, malicious_total_list, malicious_list, normal_total_cnt, normal_total_list, normal_list, step = loadResultSet(3)
    ...
    draw_confusion_matrix(y_label, y_predicted, acc, step)
    draw_roc_curve(y_label, y_predicted, step)
