...
def oneHotEncoding(categories, numerics):
    ...
    numerical_oneHotEncoded = []

    for i in range(len(normalized)):
        temp = []

        for j in range(len(normalized[0])):
            if 0.0 <= normalized[i][j] < 0.1:
                for k in range(10):
                    if k == 0:
                        temp.append(1)
                    else:
        ...
        numerical_oneHotEncoded.append(temp)
        del temp

    print("After numeric value one hot encoding: " + str(len(numerical_oneHotEncoded[0])))
    ...
    for i in range(len(normalized)):
        ...
        for j in range(len(numerical_oneHotEncoded[i][0:30])):
            temp.append(numerical_oneHotEncoded[i][j])

        for j in range(len(encoded_category3[i])):
            temp.append(encoded_category3[i][j])

        for j in range(len(numerical_oneHotEncoded[i][30:])):
            temp.append(numerical_oneHotEncoded[i][j])
...
def saveDataset(data):
    ...
    training_set = data[0:num_training]
